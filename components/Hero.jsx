...
export default function Hero({ isDarkMode, onReservasi }) {
  ...
  // Smooth mouse move using spring
  ...
  return (
    <section ref={containerRef} className={`relative h-screen w-full flex items-center justify-center overflow-hidden pt-20 transition-colors duration-500 ${isDarkMode ? 'bg-brand-black' : 'bg-brand-white'}`}>
      ...
      <motion.div 
        style={{ y, scale, x: mouseX, rotate: useTransform(mouseX, [-25, 25], [-1, 1]) }}
        className="absolute inset-0 z-0"
      >
        <div className={`absolute inset-0 z-10 transition-colors duration-700 ${gradientClass}`} />
        <img 
          ...
          className={`w-full h-full object-cover transition-all duration-700 ${isDarkMode ? 'brightness-[0.3] saturate-[0.5]' : 'brightness-110 saturate-[0.8]'}`}
        />
      </motion.div>
      ...
      <div className="relative z-20 w-full max-w-[1800px] px-6 md:px-12 flex flex-col items-center text-center">
        ...
        <div className="overflow-visible mb-2">
          <div className="overflow-hidden px-4 -mx-4 pt-16 -mt-16 pb-4">
            ...
            >
              LEVEL <span className={`${accentColor} italic inline-block pr-4`}>UP.</span>
            </motion.h1>
          </div>
        </div>
        ...
        >
          <p className={`max-w-2xl text-xl md:text-4xl font-display uppercase tracking-widest ${isDarkMode ? 'text-brand-blue/60' : 'text-brand-pink/70'}`}>
      ...
      <motion.div 
        style={{ opacity }}
        className={`absolute bottom-10 left-12 hidden md:flex items-center gap-4 text-xs uppercase tracking-[0.3em] ${isDarkMode ? 'text-brand-blue/30' : 'text-brand-pink/30'}`}
      >
        <div className={`w-px h-12 relative overflow-hidden ${isDarkMode ? 'bg-brand-blue/20' : 'bg-brand-pink/20'}`}>
          <motion.div 
            animate={{ y: ["-100%", "100%"] }}
            transition={{ repeat: Infinity, duration: 1.5, ease: "linear" }}
            className={`absolute inset-0 ${isDarkMode ? 'bg-brand-blue' : 'bg-brand-pink'}`}
          />
        </div>
